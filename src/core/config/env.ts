...
const readInt = (name: string, fallback: number) => {
  ...
  if (!Number.isInteger(value) || value <= 0) throw new AppError('CONFIG_INVALID', `${name} must be a positive integer`, 500);
  return value;
};
...
export type AppConfig = Readonly<{
  ...
  backupSecret: string;
  sessionHours: number;
  allowStandaloneSetup: boolean;
  ownerCenterUrl?: string;
  ownerProductCode: string;
  drugMasterAutoSeed: boolean;
  drugMasterSeedFile: string;
  alertRefreshMinutes:number;
  cleanupHours:number;
}>;
...
export function loadConfig(): AppConfig {
  ...
  if (!databaseUrl) throw new AppError('CONFIG_DATABASE_URL_REQUIRED', 'DATABASE_URL is required', 500);
  if (appSecret.length < 32) throw new AppError('CONFIG_APP_SECRET_WEAK', 'APP_SECRET must be at least 32 characters', 500);
  if (backupSecret.length < 32) throw new AppError('CONFIG_BACKUP_SECRET_WEAK', 'BACKUP_SECRET must be at least 32 characters', 500);
