...
import type { ReplenishmentService } from '../application/replenishment-service.js';

export const replenishmentRoutes = (service: ReplenishmentService, organization: OrganizationContract) => {
  const router = Router();
  ...
  router.get('/recommendations', requirePermission('purchases.read'), asyncHandler(async (req, res) => {
    ...
    if (!branchId) throw new AppError('BRANCH_REQUIRED', 'الفرع مطلوب', 422);
