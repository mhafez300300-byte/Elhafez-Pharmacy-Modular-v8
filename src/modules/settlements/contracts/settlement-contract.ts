...
export type PartyCreditView=Readonly<{id:string;tenantId:string;partyType:PartyType;partyId:string;referenceType:string;referenceId:string;originalAmount:number;balance:number;status:'open'|'partial'|'settled';createdAt:string}>;
...
export type ReferenceReduction=Readonly<{applied:number;excess:number;creditId:string|null;partyType:PartyType;partyId:string}>;
export interface SettlementContract{
 ...
 reduceObligationByReference(input:{tenantId:string;partyType:PartyType;partyId:string;referenceType:string;referenceId:string;amount:number;creditReferenceType?:string;creditReferenceId?:string},tx:DbTx):Promise<ReferenceReduction>;
 ...
 listCredits(tenantId:string,partyType?:PartyType,partyId?:string):Promise<PartyCreditView[]>;
 creditBalance(tenantId:string,partyType:PartyType,partyId:string,tx?:DbTx):Promise<number>;
