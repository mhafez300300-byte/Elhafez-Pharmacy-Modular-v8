...
import type { DrugMasterContract } from '../contracts/drug-master-contract.js';
import type { DrugMasterService } from '../application/drug-master-service.js';

export const drugMasterRoutes=(service:DrugMasterService,repo:DrugMasterContract)=>{
  const r=Router();
  r.use(requireAuth);
  ...
  r.post('/:id/adopt',requirePermission('catalog.manage'),asyncHandler(async(req,res)=>{
    const input:{sellingPrice?:number;costPrice?:number;reorderLevel?:number;taxRate?:number}={};
    if(req.body?.sellingPrice!=null)input.sellingPrice=Number(req.body.sellingPrice);
    if(req.body?.costPrice!=null)input.costPrice=Number(req.body.costPrice);
    if(req.body?.reorderLevel!=null)input.reorderLevel=Number(req.body.reorderLevel);
    if(req.body?.taxRate!=null)input.taxRate=Number(req.body.taxRate);
