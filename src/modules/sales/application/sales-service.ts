...
import type { TrackTraceRecorder } from '../../tracktrace/contracts/tracktrace-contract.js';
import type { SettingsContract } from '../../settings/contracts/settings-contract.js';
...
export class SalesService{
  constructor(
    ...
    private readonly trackTrace?:TrackTraceRecorder,
  ...
  async post(tenantId:string,userId:string,input:SaleInput):Promise<SaleView>{
    ...
    return this.uow.withTransaction(async tx=>{
      ...
      const salePrefs=this.settings?await this.settings.get(tenantId,tx):null;if(this.trackTrace&&salePrefs?.preferences.trackTraceEnabled!==false&&salePrefs?.preferences.trackTraceAutoSales!==false)for(const line of lines)for(const a of line.allocations)await this.trackTrace.recordDispense(tenantId,userId,{branchId:input.branchId,productId:line.productId,batchId:a.batchId,batchNo:a.batchNo??null,quantity:a.quantity,saleId:id},tx);
  ...
  async returnSale(tenantId:string,userId:string,input:{saleId:string;lines:ReadonlyArray<{saleLineId:string;quantity:number;classification:'sellable'|'quarantine'|'damaged'|'expired'}>;reason?:string}){
    return this.uow.withTransaction(async tx=>{
      ...
      for(const item of input.lines){
        ...
        const ratio=item.quantity/line.quantity,lineShare=lineNetTotal>0?line.net/lineNetTotal:0,amount=round2(sale.total*lineShare*ratio);
        total=round2(total+amount);
        let remaining=item.quantity,skip=already,lineCost=0;
        for(const alloc of line.allocations){
          if(remaining<=1e-9)break;
          const allocQty=Number(alloc.quantity);
          if(skip>=allocQty-1e-9){skip-=allocQty;continue;}
          const available=Math.max(0,allocQty-skip),qty=Math.min(remaining,available);skip=0;
          if(qty<=1e-9)continue;
          lineCost=round2(lineCost+qty*Number(alloc.unitCost));
        ...
        if(remaining>1e-9)throw new AppError('RETURN_BATCH_ALLOCATION_INVALID','تعذر ربط المرتجع بالتشغيلات الأصلية بالكامل',409,{saleLineId:line.id,remaining});
