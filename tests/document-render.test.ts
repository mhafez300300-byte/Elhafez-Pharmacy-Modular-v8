import test from'node:test';import assert from'node:assert/strict';import{escapeHtml,escapeXml,safeFilename}from'../src/modules/documents/domain/render-utils.js';
...
test('native PDF document path delegates clean Arabic HTML to renderer',async()=>{
  const sale={id:'sale1',number:'S-1',tenantId:'t',branchId:'b',customerId:null,userId:'u',payment:'cash',subtotal:100,discount:0,invoiceDiscount:0,loyaltyPointsRedeemed:0,loyaltyDiscount:0,loyaltyPointsEarned:10,tax:0,total:100,cost:50,profit:50,status:'posted' as const,createdAt:'2026-09-08T00:00:00.000Z',lines:[{id:'l1',productId:'p1',quantity:1,unitPrice:100,discount:0,tax:0,net:100,cost:50,allocations:[]}]};
  let htmlSeen='';
  const svc=new DocumentService(
    {get:async()=>sale} as any,
    {} as any,
    {get:async()=>({id:'p1',tenantId:'t',name:'باراسيتامول',barcode:null,sku:null,sellingPrice:100,costPrice:50,taxRate:0,reorderLevel:0,requiresPrescription:false,controlledClass:null,active:true})} as any,
    {} as any,{} as any,
    {get:async()=>({tenantId:'t',pharmacyName:'صيدلية الاختبار',phone:null,address:null,invoiceFooter:null})} as any,
    {getTenant:async()=>({id:'t',name:'صيدلية الاختبار',status:'trial',currency:'EGP'}),listBranches:async()=>[{id:'b',tenantId:'t',name:'الرئيسي',active:true}]} as any,
    {renderHtml:async(html:string)=>{htmlSeen=html;return new Uint8Array([37,80,68,70,45,49,46,55]);}} as any,
  );
  const pdf=await svc.salePdf('t','sale1');
  assert.equal(pdf.contentType,'application/pdf');
  assert.ok(pdf.filename.endsWith('.pdf'));
  assert.deepEqual(Array.from(pdf.content as Uint8Array).slice(0,4),[37,80,68,70]);
