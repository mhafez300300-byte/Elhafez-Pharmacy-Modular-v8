...
test('shortage notebook rejects a recent duplicate before creating another row',async()=>{
  const uow:any={withTransaction:async(fn:any)=>fn(tx)};
  const repo:any={findRecentDuplicate:async()=>({id:'shr-existing'}),create:async()=>{throw new Error('must not create')}};
  const service=new ShortageService(uow,repo,{} as any);
  await assert.rejects(()=>service.create('t1','u1',{branchId:'b1',freeText:'دواء غير موجود',quantity:1,source:'pos_not_found'}),(e:any)=>e?.code==='SHORTAGE_DUPLICATE');
});
