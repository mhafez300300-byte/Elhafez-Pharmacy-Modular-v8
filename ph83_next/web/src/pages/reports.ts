...
export async function reportsPage(){
  ...
  const [sales,alerts,health,suppliers,balances,returns]=await Promise.all([
    api<any>('/api/reports/sales'),api<any>(`/api/reports/stock-alerts?branchId=${branch}`),api<any>(`/api/reports/stock-health?branchId=${branch}`),api<any>('/api/reports/supplier-performance'),api<any>('/api/reports/party-balances'),api<any>('/api/reports/returns'),
  ]);
  return h('div',{},
    h('div',{class:'section-head'},h('div',{},h('h2',{},'التقارير الاحترافية'),h('p',{},'مبيعات، صحة مخزون، موردون، ذمم ومرتجعات من بيانات النظام الفعلية'))),
    h('div',{class:'grid two'},
      section('ملخص المبيعات اليومي',sales.items?.length?salesTable(sales.items,sales.profitVisible===true):empty('لا توجد مبيعات بعد.')),
      section('قرب انتهاء الصلاحية',alerts.items?.length?alertsTable(alerts.items):empty('لا توجد تنبيهات صلاحية.')),
    ),
    ...
    h('div',{class:'grid two section-gap'},
      section('أداء الموردين',suppliers.items?.length?supplierTable(suppliers.items,suppliers.costVisible===true):empty('لا توجد بيانات موردين.')),
      section('أرصدة الأطراف',balanceTable(balances)),
    ),
    section('تحليل المرتجعات',returnTable(returns),'section-gap'),
  );
}
