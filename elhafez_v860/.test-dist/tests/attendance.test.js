...
test('attendance check-in and check-out are audited in one transaction', async () => {
    ...
    const repo = {
        getOpen: async () => open,
        checkIn: async (input) => { calls.push('check-in'); open = { ...input, checkIn: '2026-09-08T00:00:00.000Z', checkOut: null }; return open; },
        checkOut: async () => { calls.push('check-out'); open = { ...open, checkOut: '2026-09-08T08:00:00.000Z' }; return open; },
        history: async () => [],
    };
    const org = { getDefaultBranch: async () => ({ id: 'b1', tenantId: 't1', name: 'الرئيسي', active: true }), listBranches: async () => [] };
    const audit = { record: async (input) => calls.push(input.action) };
