export class PostgresReportRepository {
    ...
    async dashboard(t, b) {
        const [sales, purchases, stock, alerts] = await Promise.all([
            this.db.query(`SELECT COALESCE(sum(total),0)::float total,count(*)::int count FROM sales_invoices WHERE tenant_id=$1 AND branch_id=$2 AND created_at::date=current_date`, [t, b]),
            this.db.query(`SELECT COALESCE(sum(total),0)::float total,count(*)::int count FROM purchase_receipts WHERE tenant_id=$1 AND branch_id=$2 AND created_at::date=current_date`, [t, b]),
            this.db.query(`SELECT COALESCE(sum(quantity*unit_cost),0)::float cost FROM inv_batches WHERE tenant_id=$1 AND branch_id=$2 AND status='sellable'`, [t, b]),
            this.db.query(`SELECT count(*)::int count FROM inv_batches WHERE tenant_id=$1 AND branch_id=$2 AND status='sellable' AND quantity>0 AND expiry_date IS NOT NULL AND expiry_date<=current_date+interval '90 days'`, [t, b]),
        ]);
    ...
    async returnAnalysis(t, from, to) {
