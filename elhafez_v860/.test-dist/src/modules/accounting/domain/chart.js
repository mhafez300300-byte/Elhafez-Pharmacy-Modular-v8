...
export function assertPeriodRange(fromDate, toDate) {
    const from = new Date(`${fromDate}T00:00:00Z`), to = new Date(`${toDate}T00:00:00Z`);
    if (Number.isNaN(from.getTime()) || Number.isNaN(to.getTime()) || from > to)
        throw new AppError('ACCOUNTING_PERIOD_INVALID', 'الفترة المحاسبية غير صحيحة', 422);
}
export function statementBucket(type) {
    if (type === 'revenue' || type === 'contra_revenue' || type === 'expense' || type === 'cogs')
        return 'income';
    return 'balance';
}
