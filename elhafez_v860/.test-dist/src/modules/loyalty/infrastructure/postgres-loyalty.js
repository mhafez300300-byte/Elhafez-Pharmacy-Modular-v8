export class PostgresLoyaltyRepository {
    ...
    async getAccount(tenantId, customerId, tx) {
        const q = await this.ex(tx).query(`SELECT tenant_id as "tenantId",customer_id as "customerId",points::float,updated_at::text as "updatedAt" FROM loy_accounts WHERE tenant_id=$1 AND customer_id=$2`, [tenantId, customerId]);
        return q.rows[0] ?? { tenantId, customerId, points: 0, updatedAt: new Date().toISOString() };
    }
    async lockAccount(tenantId, customerId, tx) {
    ...
    async history(tenantId, customerId, limit = 100) {
