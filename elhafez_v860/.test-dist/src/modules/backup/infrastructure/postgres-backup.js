...
const EPHEMERAL_CLEAR = ['id_sessions', 'id_login_attempts', 'idem_keys'];
...
export class PostgresBackupRepository {
    ...
    async exportSnapshot() {
        ...
        for (const name of TABLE_ORDER) {
            const exists = await this.db.query(`SELECT to_regclass($1) IS NOT NULL AS ok`, [name]);
            if (!exists.rows[0]?.ok)
                continue;
            const rows = (await this.db.query(`SELECT * FROM ${qi(name)}`)).rows;
            tables.push({ name, rows });
        }
        return { format: 'ELHAFEZ_PHARMACY_DB_SNAPSHOT', version: 1, appVersion: '8.6.0', createdAt: new Date().toISOString(), tables };
    }
    async restoreSnapshot(snapshot, tx) {
        if (snapshot.format !== 'ELHAFEZ_PHARMACY_DB_SNAPSHOT' || snapshot.version !== 1)
            throw new AppError('BACKUP_FORMAT_INVALID', 'ملف النسخة الاحتياطية غير متوافق', 422);
        const allowed = new Set(TABLE_ORDER);
        const byName = new Map(snapshot.tables.map(t => [t.name, t]));
        for (const table of snapshot.tables)
            if (!allowed.has(table.name))
                throw new AppError('BACKUP_TABLE_INVALID', 'ملف النسخة الاحتياطية يحتوي جدولاً غير مسموح', 422, { table: table.name });
        const present = TABLE_ORDER.filter(name => byName.has(name));
        if (!present.includes('org_tenants') || !present.includes('id_users'))
            throw new AppError('BACKUP_INCOMPLETE', 'النسخة الاحتياطية غير مكتملة', 422);
        // Restore is intentionally whole-instance. All application data is replaced atomically.
        const existing = await tx.query(`SELECT tablename AS name FROM pg_tables WHERE schemaname=current_schema()`);
        ...
        if (toClear.length)
            await tx.query(`TRUNCATE ${toClear.map(qi).join(', ')} RESTART IDENTITY CASCADE`);
        for (const name of TABLE_ORDER) {
