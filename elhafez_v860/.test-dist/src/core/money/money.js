...
export function moneyFromDecimal(value, currency = 'EGP', scale = 2) {
    if (scale < 0 || scale > 4)
        throw new AppError('MONEY_SCALE_INVALID', 'Unsupported money scale');
    const normalized = typeof value === 'number' ? value.toFixed(scale) : String(value).trim();
    if (!/^-?\d+(\.\d+)?$/.test(normalized))
        throw new AppError('MONEY_INVALID', 'Invalid monetary value');
...
export function moneyToDecimal(value, scale = 2) {
    const factor = POW10[scale] ?? 100n;
...
export const addMoney = (a, b) => {
    if (a.currency !== b.currency)
        throw new AppError('MONEY_CURRENCY_MISMATCH', 'Currency mismatch');
