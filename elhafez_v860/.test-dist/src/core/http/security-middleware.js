...
export const sameOriginWriteGuard = (req, _res, next) => {
    ...
    if (!origin)
        return next();
    try {
        const parsed = new URL(String(origin)), host = String(req.headers.host ?? '');
        if (parsed.host !== host)
            throw new Error('host');
        return next();
    }
    catch {
        return next(new AppError('ORIGIN_REJECTED', 'تم رفض مصدر الطلب للحماية من الطلبات غير الموثوقة', 403));
    }
};
