...
export async function dashboardPage() {
    ...
    const [data, health, balances, alerts, settings, auditHealth, recon] = await Promise.all([
        ...
        api('/api/audit/verify').catch(() => ({ ok: false, status: 'unavailable' })),
        api('/api/reconciliation').catch(() => ({ ok: false, issues: [{ code: 'UNAVAILABLE' }] })),
    ]);
