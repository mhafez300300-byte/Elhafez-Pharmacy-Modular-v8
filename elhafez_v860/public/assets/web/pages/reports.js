...
export async function reportsPage() {
    ...
    async function load() {
        ...
        content.replaceChildren(h('div', { class: 'loading-state', role: 'status' }, 'جارٍ تجهيز التقارير…'));
        try {
            ...
            const [sales, alerts, health, suppliers, balances, returns] = await Promise.all([
                api(`/api/reports/sales?${q}`), api(`/api/reports/stock-alerts?branchId=${branch}`), api(`/api/reports/stock-health?branchId=${branch}`), api('/api/reports/supplier-performance'), api('/api/reports/party-balances'), api(`/api/reports/returns?${q}`),
            ]);
            exportRows = [['التاريخ', 'عدد الفواتير', 'المبيعات', 'الربح'], ...(sales.items ?? []).map((x) => [String(x.day).slice(0, 10), String(x.invoices), String(x.total), sales.profitVisible === true ? String(x.profit) : ''])];
            content.replaceChildren(h('div', { class: 'grid two' }, section('ملخص المبيعات', sales.items?.length ? salesTable(sales.items, sales.profitVisible === true) : empty('لا توجد مبيعات في الفترة.')), section('قرب انتهاء الصلاحية', alerts.items?.length ? alertsTable(alerts.items) : empty('لا توجد تنبيهات صلاحية.'))), section('صحة المخزون', health.items?.length ? stockHealthTable(health.items, health.costVisible === true) : empty('لا توجد أصناف.'), 'section-gap'), h('div', { class: 'grid two section-gap' }, section('أداء الموردين', suppliers.items?.length ? supplierTable(suppliers.items, suppliers.costVisible === true) : empty('لا توجد بيانات موردين.')), section('أرصدة الأطراف', balanceTable(balances))), section('تحليل المرتجعات', returnTable(returns), 'section-gap'));
    ...
    run.onclick = () => void load();
    printBtn.onclick = () => window.print();
    exportBtn.onclick = () => downloadCsv(exportRows, `sales-report-${from.value}-${to.value}.csv`);
    const root = h('div', {}, h('div', { class: 'section-head' }, h('div', {}, h('h2', {}, 'التقارير التشغيلية'), h('p', {}, 'فلاتر زمنية + أرقام قابلة للتصدير والمراجعة')), h('div', { class: 'section-actions' }, h('label', { class: 'field compact' }, h('span', {}, 'من'), from), h('label', { class: 'field compact' }, h('span', {}, 'إلى'), to), run, exportBtn, printBtn)), stamp, content);
    await load();
    return root;
}
