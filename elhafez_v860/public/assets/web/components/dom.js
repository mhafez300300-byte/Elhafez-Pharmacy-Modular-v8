...
export function h(tag, attrs = {}, ...children) {
    ...
    for (const [k, v] of Object.entries(attrs)) {
        ...
        if (propertySafe.has(k)) {
            try {
                el[k] = v;
                continue;
            }
            catch { /* attribute fallback */ }
        }
        if (v === true)
...
export function toast(message, error = false) { document.querySelector('.toast')?.remove(); const el = h('div', { class: `toast${error ? ' error' : ''}`, role: error ? 'alert' : 'status' }, message); document.body.append(el); setTimeout(() => el.remove(), 3500); }
export function modal(title, body, onSubmit) {
    ...
    backdrop.addEventListener('click', e => { if (e.target === backdrop)
        dismiss(); });
    form.addEventListener('submit', async (e) => { e.preventDefault(); if (save.disabled)
        return; try {
        save.disabled = true;
        ...
        toast(err?.message ?? 'تعذر الحفظ', true);
        save.disabled = false;
