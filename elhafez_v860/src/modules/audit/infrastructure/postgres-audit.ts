...
export class PostgresAuditRepository implements AuditContract{
  ...
  async verifyChain(tenantId:string){
    const rows=(await this.db.query<any>(`SELECT id,user_id as "userId",action,entity,entity_id as "entityId",detail,created_at::text as "createdAt",prev_hash as "prevHash",event_hash as "eventHash" FROM audit_events WHERE tenant_id=$1 ORDER BY id`,[tenantId])).rows;
    let prev:string|null=null,checked=0,unhashed=0,firstBrokenId:number|null=null;
    for(const r of rows){
      if(!r.eventHash){unhashed++;continue;}
      ...
      if(r.prevHash!==prev||r.eventHash!==expected){firstBrokenId=Number(r.id);break;}
      prev=r.eventHash;checked++;
    }
    const head=(await this.db.query<{lastHash:string|null}>(`SELECT last_hash as "lastHash" FROM audit_heads WHERE tenant_id=$1`,[tenantId])).rows[0]?.lastHash??null;
