...
export async function notificationsPage() {
  ...
  const refresh = h('button', { class: 'btn primary', type: 'button' }, 'تحديث التنبيهات') as HTMLButtonElement;

  root.append(
    h('div', { class: 'section-head' },
      h('div', {},
        h('h2', {}, 'مركز التنبيهات'),
        h('p', {}, 'نفاد وانخفاض المخزون والصلاحية — تنبيهات قابلة للتنفيذ بدون تكرار عشوائي'),
      ),
      refresh,
    ),
    box,
  ...
  async function load(doRefresh = false) {
    if (doRefresh) await post('/api/alerts/refresh', {});
    const d = await api<any>('/api/notifications');
    const list = h('div', { class: 'list' });
    for (const n of d.items ?? []) {
      const read = !!n.readAt;
      const btn = h('button', { class: 'btn sm', type: 'button', disabled: read }, read ? 'تمت القراءة' : 'تحديد كمقروء') as HTMLButtonElement;
      if (!read) {
        btn.onclick = async () => {
      ...
      actions.append(btn);
      list.append(
        h('div', { class: `alert-row ${n.severity || 'info'} ${read ? 'read' : ''}` },
          h('div', {},
            h('strong', {}, n.title),
            h('p', {}, n.body),
            h('small', { class: 'muted' }, new Date(n.createdAt).toLocaleString('ar-EG')),
          ),
          actions,
        ),
      );
