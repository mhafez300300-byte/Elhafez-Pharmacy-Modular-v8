...
const rolePages = { cashier: new Set(['dashboard', 'pos', 'sales', 'customers', 'cash', 'shortages']), pharmacist: new Set(['dashboard', 'pos', 'sales', 'products', 'inventory', 'expiry', 'drug_master', 'customers', 'cash', 'shortages', 'track_trace', 'insurance']) };
function visibleNavItems() { const role = String(state.user?.role ?? '').toLowerCase(), allowed = rolePages[role]; return allowed ? navItems.filter(([p]) => allowed.has(p)) : navItems; }
function enhanceTables(root) { for (const table of root.querySelectorAll('table')) {
    const heads = [...table.querySelectorAll('thead th')].map(x => x.textContent?.trim() || '');
    for (const row of table.querySelectorAll('tbody tr'))
        [...row.children].forEach((cell, i) => { if (cell instanceof HTMLElement && heads[i])
...
function renderApp() { const skip = h('a', { class: 'skip-link', href: '#pageContent' }, 'تخطي إلى المحتوى الرئيسي'); const side = h('aside', { class: 'sidebar' }, h('div', { class: 'sidebar-brand' }, h('div', { class: 'brand-mark' }, 'E'), h('div', {}, h('strong', {}, 'Elhafez Pharmacy'), h('small', {}, 'v8.4.0 INTELLIGENCE'))), h('nav', { class: 'nav accordion-nav' }), h('div', { class: 'sidebar-footer' }, h('div', {}, state.user?.name ?? ''), h('div', {}, state.branch?.name ?? ''))); const nav = side.querySelector('.nav'); const currentNavItems = visibleNavItems(), groups = [...new Set(currentNavItems.map(([p]) => navGroups[p] ?? ''))]; let openGroup = navGroups[state.page] ?? groups[0] ?? ''; const renderNav = () => { nav.replaceChildren(...groups.map(group => { const items = currentNavItems.filter(([p]) => (navGroups[p] ?? '') === group), open = group === openGroup, box = h('div', { class: `nav-group ${open ? 'open' : ''}` }), header = h('button', { class: 'nav-group-toggle', type: 'button', 'aria-expanded': String(open), onClick: () => { openGroup = open ? '' : group; renderNav(); } }, h('span', { class: 'nav-group-title' }, group), h('span', { class: 'nav-chevron' }, '⌄')), body = h('div', { class: 'nav-group-items' }); for (const [p, l, g] of items)
    body.append(h('button', { type: 'button', 'data-page': p, class: state.page === p ? 'active' : '', onClick: () => void navigate(p) }, h('span', { class: 'nav-glyph' }, g), h('span', {}, l))); box.append(header, body); return box; })); }; renderNav(); const commandBtn = h('button', { class: 'btn sm command-button', type: 'button', onClick: () => openCommandPalette() }, '⌘ انتقال سريع'); const alertBtn = h('button', { class: 'btn sm alert-button', type: 'button', onClick: () => void navigate('notifications') }, 'تنبيهات', h('span', { class: 'alert-badge', id: 'alertBadge' }, '0')); const main = h('main', { class: 'main' }, h('header', { class: 'topbar' }, h('div', { style: 'display:flex;align-items:center;gap:10px' }, h('button', { class: 'btn sm mobile-menu', onClick: () => side.classList.toggle('open') }, '☰'), h('div', { class: 'page-title' }, h('strong', { id: 'pageTitle' }, ''), h('small', { id: 'pageSubtitle' }, ''))), h('div', { class: 'top-actions' }, h('span', { class: 'chip' }, state.branch?.name ?? 'بدون فرع'), commandBtn, alertBtn, h('button', { class: 'btn sm', onClick: async () => { await post('/api/auth/logout', {}).catch(() => { }); state.user = null; renderLogin(); } }, 'خروج'))), h('section', { class: 'content', id: 'pageContent' })); mount.replaceChildren(skip, h('div', { class: 'app-layout' }, side, main)); void navigate(state.page); void refreshUnreadBadge(); }
function openCommandPalette() { const input = h('input', { class: 'search-input', placeholder: 'اكتب: بيع، مخزون، شراء، عميل، تقرير…', autofocus: true }), results = h('div', { class: 'command-results' }), backdrop = h('div', { class: 'modal-backdrop command-backdrop', onClick: (e) => { if (e.target === backdrop)
        backdrop.remove(); } }, h('div', { class: 'modal command-modal' }, h('div', { class: 'modal-head' }, h('div', {}, h('h3', {}, 'انتقال سريع'), h('small', { class: 'muted' }, 'اذهب لأي وظيفة بدون البحث في القوائم')), h('button', { class: 'icon-close', type: 'button', onClick: () => backdrop.remove() }, '×')), h('div', { class: 'modal-body' }, input, results))); const render = () => { const q = input.value.trim(); const list = visibleNavItems().filter(([p, l]) => !q || `${p} ${l} ${labels[p]?.join(' ') ?? ''}`.includes(q)).slice(0, 12); results.replaceChildren(...list.map(([p, l, g]) => h('button', { class: 'command-result', onClick: () => { backdrop.remove(); void navigate(p); } }, h('span', { class: 'command-glyph' }, g), h('span', {}, h('b', {}, l), h('small', {}, labels[p]?.[1] ?? '')), h('kbd', {}, '↵')))); }; input.oninput = render; input.onkeydown = (e) => { if (e.key === 'Escape')
...
async function navigate(page) { state.page = page; document.querySelectorAll('.nav button').forEach(x => x.classList.toggle('active', x.dataset.page === page)); document.querySelector('.sidebar')?.classList.remove('open'); const [title, sub] = labels[page] ?? [page, '']; (document.getElementById('pageTitle')).textContent = title; (document.getElementById('pageSubtitle')).textContent = sub; const content = document.getElementById('pageContent'); content.replaceChildren(h('div', { class: 'empty' }, 'جاري التحميل…')); try {
    ...
    content.replaceChildren(await loader());
