...
function renderApp() { const side = h('aside', { class: 'sidebar' }, h('div', { class: 'sidebar-brand' }, h('div', { class: 'brand-mark' }, 'E'), h('div', {}, h('strong', {}, 'Elhafez Pharmacy'), h('small', {}, 'v8.2.0 PREMIUM'))), h('nav', { class: 'nav accordion-nav' }), h('div', { class: 'sidebar-footer' }, h('div', {}, state.user?.name ?? ''), h('div', {}, state.branch?.name ?? ''))); const nav = side.querySelector('.nav'); const groups = [...new Set(navItems.map(([p]) => navGroups[p] ?? ''))]; let openGroup = navGroups[state.page] ?? groups[0] ?? ''; const renderNav = () => { nav.replaceChildren(...groups.map(group => { const items = navItems.filter(([p]) => (navGroups[p] ?? '') === group), open = group === openGroup, box = h('div', { class: `nav-group ${open ? 'open' : ''}` }), header = h('button', { class: 'nav-group-toggle', type: 'button', 'aria-expanded': String(open), onClick: () => { openGroup = open ? '' : group; renderNav(); } }, h('span', { class: 'nav-group-title' }, group), h('span', { class: 'nav-chevron' }, '⌄')), body = h('div', { class: 'nav-group-items' }); for (const [p, l, g] of items)
