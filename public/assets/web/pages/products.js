...
export async function productsPage() {
    const root = h('div'), head = h('div', { class: 'section-head' }, h('div', {}, h('h2', {}, 'الأصناف'), h('p', {}, 'تعريف المنتجات والأسعار والباركود + قاعدة أدوية مركزية نظيفة')), h('div', { class: 'section-actions' }, h('button', { class: 'btn', id: 'masterBtn' }, 'قاعدة الأدوية المركزية'), h('button', { class: 'btn primary' }, '+ صنف جديد')));
    const card = h('section', { class: 'card' }), search = h('input', { class: 'search-input', placeholder: 'بحث بالاسم أو الباركود أو SKU' }), table = h('div', { class: 'table-wrap' });
    ...
    async function loadMaster() { const q = master.querySelector('#masterSearch').value.trim(); const result = master.querySelector('#masterResults'); result.replaceChildren(h('div', { class: 'empty' }, 'جاري البحث…')); try {
        ...
        result.replaceChildren(data.items?.length ? masterTable(data.items, load) : h('div', { class: 'empty' }, 'لا توجد نتائج.'));
    }
    catch (e) {
        result.replaceChildren(h('div', { class: 'auth-error' }, e.message ?? 'تعذر البحث'));
    } }
    search.oninput = () => void load();
    head.querySelector('.primary').onclick = () => openAdd(load);
    head.querySelector('#masterBtn').onclick = () => { master.classList.toggle('hidden'); if (!master.classList.contains('hidden'))
        void loadMaster(); };
    master.querySelector('#masterRefresh').onclick = () => void loadMaster();
    master.querySelector('#masterSearch').onkeydown = e => { if (e.key === 'Enter') {
        e.preventDefault();
        void loadMaster();
    } };
    await load();
...
function openAdopt(d, done) { const body = h('div', { class: 'form-grid' }, h('div', { class: 'auth-note span-2' }, `${d.nameAr}${d.manufacturer ? ' — ' + d.manufacturer : ''}`), num('sellingPrice', 'سعر البيع', d.officialPrice ?? 0), num('costPrice', 'التكلفة', 0), num('reorderLevel', 'حد إعادة الطلب', 0), num('taxRate', 'الضريبة %', 0)); modal('إضافة الدواء إلى أصناف الصيدلية', body, async (form) => { const fd = new FormData(form); await post(`/api/drug-master/${d.id}/adopt`, { sellingPrice: Number(fd.get('sellingPrice')), costPrice: Number(fd.get('costPrice')), reorderLevel: Number(fd.get('reorderLevel')), taxRate: Number(fd.get('taxRate')) }); toast('تم إنشاء الصنف من قاعدة الأدوية المركزية'); await done(); }); }
